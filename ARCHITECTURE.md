...
更新：2026-09-06。本目录只维护当前可玩的 Godot 4.7.1 / .NET 8 demo。
...
└─ _release/                   Windows 发行包与校验文件，本次版本 v0.1.5
```
...
所有 Label / RichTextLabel 使用同一个点阵材质，24–34 px 为主要文字范围；点阵覆盖率下限为 0.44、亮度 1.24、微光 0.035。按钮的原生文字透明，场景中单独的 `MatrixCaption` 子节点负责显示；其 `MatrixButtonCaption` 脚本只同步文字，不创建布局或资源。文字 shader 不影响按钮底色。

三个选项保留绿 / 黄 / 红边框与对应文字配色。普通和禁用状态背景透明，悬浮及选中状态保留填色；A / B / C 第一行不随选择改变。确认文字的场景锚点和双向对齐保证居中。禁用 X 保持高亮，RGB 与各自外框相同，分别为绿 / 黄 / 红 / 蓝，且 X 位于文字图层上方。边框 `speed` 仍为 0.04，即一周约 25 秒。

`LatticeView.OrbitScale` 在场景中设为 1.12；`PlayerPreview` 为金色、`TutorPreview` 为绿色。`GameplayHUD.RevealNumberHoldSeconds` 为 3 秒，数字在前 70% 时间保持不透明，再逐渐消失。`UiAudioController.ExtractionSound` 和独立 `ExtractionPlayer` 在主场景绑定，Bash 双方及 Limit 共同移入动画在开始时播放，按移动时长匹配播放速度。原始音效可用 `tools/generate_extraction_sfx.py` 重建。
...
- Limit 对话面板在选择前显示 `TutorSealedText`，确认后显示 `BothSealedText`，揭示时隐藏。它们是场景可编辑的剧情提示，实际合法动作求解仍由原有流程在玩家锁定后执行。揭示解释复用已有中性录音，字幕和语音不再提及旧版青色节点。
...
v0.1.4 发行包包含 487 个 PCK 资源、190 个运行文件，已检查排除测试、编辑器缓存和玩家进度，且解压副本逐文件校验一致。独立解压目录中的 Windows EXE 分别以 D3D12 和 OpenGL 启动，均以 0 退出且无警告 / 错误；玩家原有存档校验值保持不变。`FullFlowSmoke` 保留正式语音和动画速度，等待上限覆盖多局流程，不通过游戏内的快速模式跳过演出。

### v0.1.5 发布验证

按本次要求，仅运行专用 `ReleaseV015Smoke.tscn` 冒烟场景，不重跑领域断言和完整对局回归。覆盖空存档隐藏 Continue、全部文字点阵层、四个按钮的四种状态与同色高亮 X、首行稳定和确认居中、节点尺寸及颜色、双方锁定提示、移入音效、3 秒飘字和本回合结算。截图在 `_qa/v015-ui/`，日志在 `_qa/v015-final-smoke.log`。

导出规则额外排除 `*demo_save*`、`*profile.json`、`*checkpoint.json`、`*.bak`、`*.tmp`。发布包只包含运行内容，空存档冒烟使用唯一测试路径，不复制开发者进度。已有玩家在系统用户目录中的存档不属于发行包内容。
