...
public sealed class BashGame
{
    ...
    private readonly List<ChoicePair> _history = new();

    public IReadOnlyList<ChoicePair> History => _history;
    ...
    public void Restore(
        ...
        IReadOnlyList<ChoicePair>? history = null)
    {
        if (initialUnits < MinimumTake
            || remaining < 0
            || remaining > initialUnits)
    ...
    public RoundOutcome ApplyTake(Actor actor, int amount)
    {
        ...
        Remaining -= amount;
        _history.Add(new ChoicePair(
            actor == Actor.Player ? amount : 0,
            actor == Actor.Tutor ? amount : 0,
            before,
            Remaining));
