...
public partial class StabilityLatticeView : Control
{
    ...
    [Export]
    public Color ActiveHighlight { get; set; } = new("b5ccdf");
    [Export]
    ...
    [Export]
    public Color OrbitBlue { get; set; } = new("527d9e");
        ...
        float pulse)
    {
        ...
        Color field = ActiveBlue with { A = 0.055f + pulse * 0.025f };
        DrawCircle(
            center,
            fieldRadius,
            field,
        ...
        float pulse)
    {
        ...
        Color pathColor = (_limitMode ? OrbitBlue : ActiveBlue) with
        {
        ...
        Vector2 scanPosition = CalculateOrbitPosition(
            ...
            precession);
        DrawCircle(scanPosition, 2.2f, OrbitBlue with { A = 0.5f });
    }
    ...
    private void DrawAnchor(
        ...
        float depth)
    {
        Color fill = active ? ActiveHighlight : InactiveBlue;
        ...
        if (previewed)
        {
            ...
            fill = _requestLocked
                ? PlayerPreview.Lerp(Colors.White, 0.22f)
                : PlayerPreview;
        }
    ...
    private Color ResolveAnchorOutline(bool active, bool previewed)
    {
        if (previewed)
        {
            return _requestLocked ? PlayerPreview : PlayerPreview;
        }

        return active ? ActiveBlue : InactiveBlue;
    }
    ...
    private Color ResolveMarkColor(AnchorMark mark)
    {
        return mark == AnchorMark.Player ? PlayerPreview : TutorPreview;
    }
